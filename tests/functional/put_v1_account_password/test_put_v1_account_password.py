def test_put_v1_account_password(account_helper, create_user_data):
    ...
    account_helper.auth_client(login=login, password=old_password)

    new_password = old_password + '1'
    account_helper.change_password(login=login, password=old_password, new_password=new_password, email=email)
